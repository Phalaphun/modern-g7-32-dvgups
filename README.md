...
| `default-leading` | `1.5em - 0.75em` | — | **Устаревший параметр.** Сохранён для совместимости и расчёта прежнего отступа после заголовка содержания. Для межстрочного интервала основного текста используйте `body-leading`. |
| `default-body-leading` | `1.5em - 0.75em + 1mm` | `body-leading` | Дополнительное расстояние между строками обычных абзацев. Не изменяет интервалы заголовков, содержания и подписей. |
| `default-contents-leading` | `1.5em - 0.75em + 1mm` | `contents-leading` | Дополнительное расстояние между строками содержания. Задаётся независимо от межстрочного интервала основного текста. |
| `default-contents-entry-spacing` | `1.5em - 0.75em + 2mm` | `contents-entry-spacing` | Вертикальный интервал между отдельными пунктами содержания. На 1 мм больше межстрочного интервала внутри многострочного пункта по умолчанию. |
...
| `default-outline-depth` | `3` | `outline-depth` | Максимальная глубина заголовков, включаемых в содержание. Значение можно переопределить через `gost.with`. |

### Заголовки и структурные разделы

| Внутренняя константа | Значение по умолчанию | Публичный параметр | Назначение |
|---|---:|---|---|
| `default-heading-margin` | `(below: 3em-2mm, above: 3em-2mm)` | — | Отступы до и после заголовков уровней 2 и ниже. Верхний отступ применяется после обычного текста; для связки «заголовок уровня 1 → заголовок уровня 2» используется отдельный параметр. |
| `default-heading-level-1-margin` | `(below: 3em, above: 0cm)` | — | Отступы до и после заголовка первого уровня. |
| `default-heading-level-2-after-level-1-above` | `3em` | `heading-level-2-after-level-1-above` | Отступ перед заголовком второго уровня, если он расположен непосредственно после заголовка первого уровня. Позволяет настраивать эту связку независимо от заголовков после обычного текста. |
| `default-heading-following-par-top` | `14pt + 3mm` | `heading-following-par-top` | Дополнительный верхний отступ первого обычного абзаца непосредственно после заголовка любого уровня. Между соседними заголовками не применяется; для заголовков приложений сохраняются отдельные настройки. |
| `default-heading-following-par-all-levels` | `false` | `heading-following-par-all-levels` | Переключает область действия предыдущего параметра: `true` применяет увеличенный интервал после заголовка любого уровня, `false` сохраняет прежнее поведение только после заголовка первого уровня. |
...
| Внутренняя константа | Значение по умолчанию | Публичный параметр | Назначение |
|---|---:|---|---|
| `default-image-figure-margin` | `(above: 32pt, below: 32pt)` | — | Базовые внешние отступы блока рисунка, относительно которых рассчитываются контекстные интервалы. |
| `default-image-figure-gap` | `12pt` | — | Прежнее базовое расстояние между изображением и подписью. Сохранено для совместимости; новый интервал задаётся через `image-caption-gap`. |
| `default-image-after-text-gap` | `32pt + 1mm` | `image-after-text-gap` | Интервал перед рисунком после обычного абзаца; увеличен на 1 мм относительно прежних 32 пт. На новой странице верхний интервал автоматически не добавляется. |
| `default-image-caption-gap` | `12pt + 1mm` | `image-caption-gap` | Расстояние между рисунком и его подписью; увеличено на 1 мм относительно прежних 12 пт. |
| `default-image-before-text-gap` | `32pt` | `image-before-text-gap` | Интервал после подписи рисунка перед обычным текстом; сохраняет прежнее значение. |
| `default-image-after-image-gap` | `32pt - 2mm` | `image-after-image-gap` | Интервал после подписи перед следующим рисунком на той же странице; уменьшен на 2 мм. |
| `default-image-before-heading-level-2-gap` | `3em - 3mm` | `image-before-heading-level-2-gap` | Верхняя составляющая интервала после подписи перед заголовком второго уровня на той же странице; уменьшена на 1 мм относительно обычных `3em - 2mm`. |
...
|---|---:|---|
| `default-table-and-raw-figure-margin-above` | `24pt` | Базовый внешний отступ перед таблицами и листингами. После текста или однотипного элемента переопределяется отдельными контекстными параметрами таблиц и листингов. |
| `default-table-and-raw-figure-below-lines` | `2` | Базовое число строк основного шрифта после таблицы или листинга. Перед текстом или заголовком второго уровня переопределяется отдельными контекстными параметрами таблиц и листингов. |
...
|---|---|---:|---|
| `table-before-text-gap` | `default-table-before-text-gap` | `28pt + 1mm` | Интервал после обычной или длинной таблицы, если непосредственно следом начинается обычный текст; увеличен на 1 мм относительно прежних 28 пт. |
| `table-after-text-gap` | `default-table-after-text-gap` | `24pt + 2mm` | Интервал перед названием обычной или длинной таблицы, если непосредственно перед ней расположен обычный текст; увеличен на 2 мм относительно прежних 24 пт. |
...
| Параметр `gost.with` | Внутренняя константа | Значение по умолчанию | Назначение |
|---|---|---:|---|
| `listing-before-text-gap` | `default-listing-before-text-gap` | `28pt + 1mm` | Интервал после обычного или длинного листинга, если непосредственно следом начинается обычный текст; увеличен на 1 мм относительно прежних 28 пт. |
| `listing-after-text-gap` | `default-listing-after-text-gap` | `24pt + 2mm` | Интервал перед названием обычного или длинного листинга, если непосредственно перед ним расположен обычный текст; увеличен на 2 мм относительно прежних 24 пт. |
| `listing-after-listing-gap` | `default-listing-after-listing-gap` | `24pt - 2mm` | Верхняя составляющая интервала перед названием листинга, непосредственно следующего за другим листингом; уменьшена на 2 мм относительно прежних 24 пт. |
| `listing-before-heading-level-2-gap` | `default-listing-before-heading-level-2-gap` | `28pt - 4mm` | Нижняя составляющая интервала после листинга перед заголовком второго уровня; уменьшена на 4 мм относительно прежних 28 пт. Верхний отступ самого заголовка остаётся независимым. |
